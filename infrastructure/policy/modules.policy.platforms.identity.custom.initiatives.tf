...
module "mod_platforms_identity_deploy_azure_monitor_baseline_initiative" {
  ...
  management_group_id     = var.settings.identity_management_group_policies.create ? azurerm_management_group.identity[0].id : data.azurerm_management_group.identity[0].id
  merge_effects           = false
