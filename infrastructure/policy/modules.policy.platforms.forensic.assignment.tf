...
module "mod_platforms_forensic_configure_network_configuration_initiative_assignment" {
  ...
  version             = "~> 2.0"
  initiative          = module.mod_platforms_forensic_configure_network_configuration_initiative[0].initiative
  count = var.settings.forensic_management_group_policies.enabled ? 1 : 0
  assignment_scope     = var.settings.forensic_management_group_policies.create ? azurerm_management_group.forensic[0].id : data.azurerm_management_group.forensic[0].id
...
module "mod_platforms_forensic_deploy_azure_monitor_baseline_forensic_initiative_assignment" {
  source              = "azurenoops/overlays-policy/azurerm//modules/policySetAssignment/managementGroup"
  version             = "~> 2.0"
  initiative          = module.mod_platforms_forensic_deploy_azure_monitor_baseline_initiative[0].initiative
  count = var.settings.forensic_management_group_policies.enabled ? 1 : 0
  assignment_scope     = var.settings.forensic_management_group_policies.create ? azurerm_management_group.forensic[0].id : data.azurerm_management_group.forensic[0].id
  assignment_location = var.default_location
