...
module policyExemption_mg 'managementGroup/az.auth.policy.exemptions.mg.bicep' = if (empty(subscriptionId) && empty(resourceGroupName)) {
...
module policyExemption_sub 'subscription/az.auth.policy.exemptions.sub.bicep' = if (!empty(subscriptionId) && empty(resourceGroupName)) {
...
module policyExemption_rg 'resourceGroup/az.auth.policy.exemptions.rg.bicep' = if (!empty(resourceGroupName) && !empty(subscriptionId)) {
