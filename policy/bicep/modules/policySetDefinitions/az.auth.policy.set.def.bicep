...
module policySetDefinition_mg './managementGroup/az.auth.policy.set.def.mg.bicep' = if (empty(subscriptionId)) {
...
module policySetDefinition_sub './subscription/az.auth.policy.set.def.sub.bicep' = if (!empty(subscriptionId)) {
